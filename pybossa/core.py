...
def configure_app(app):
    ...
    if app.config.get("SQLALCHEMY_BINDS") is None:
        ...
        app.config["SQLALCHEMY_BINDS"] = dict(
            slave=app.config.get("SQLALCHEMY_DATABASE_URI")
        )
...
def setup_db(app):
    ...
    def create_slave_session(db, bind):
        if app.config.get("SQLALCHEMY_BINDS")["slave"] == app.config.get(
...
def setup_error_email(app):
    ...
    if not app.debug and ADMINS:  # pragma: no cover
        mail_handler = SMTPHandler(
            "127.0.0.1", "server-error@no-reply.com", ADMINS, "error"
        )
...
def setup_login_manager(app):
    ...
    if app.config.get("ENVIRONMENT") != "development":
        login_manager.login_view = (
            f'https://{app.config.get("PUBLIC_URL")}/account/signin'
        )
...
def setup_blueprints(app):
    ...
    app.register_blueprint(
        rq_dashboard.blueprint, url_prefix="/admin/rq", redis_conn=sentinel.master
    )
...
def setup_error_handlers(app):
    ...
    def handle_csrf_error(e):
        response = dict(template="400.html", code=400, description=CSRFERROR)
        return handle_content_type(response)

    @app.errorhandler(400)
    def _bad_request(e):
        response = dict(template="400.html", code=400, description=BADREQUEST)
        return handle_content_type(response)

    @app.errorhandler(404)
    def _page_not_found(e):
        response = dict(template="404.html", code=404, description=NOTFOUND)
        return handle_content_type(response)

    @app.errorhandler(500)
    def _server_error(e):  # pragma: no cover
        response = dict(template="500.html", code=500, description=INTERNALSERVERERROR)
        return handle_content_type(response)
    ...
    def _unauthorized(e):
        response = dict(template="401.html", code=401, description=UNAUTHORIZED)
        return handle_content_type(response)
...
def setup_scheduled_jobs(app):  # pragma: no cover
    ...
    from pybossa.jobs import enqueue_periodic_jobs, schedule_job, get_quarterly_date
...
def setup_upref_mdata(app):
    ...
    upref_mdata_choices = dict(languages=[], locations=[], timezones=[], user_types=[])
    if app.config.upref_mdata:
        from settings_upref_mdata import (
            upref_languages,
            upref_locations,
            mdata_timezones,
            mdata_user_types,
